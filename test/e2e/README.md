...
They are SLOW (~5-15 minutes) and require:
...
**Not run by default:** `make test-scripts` runs only fast tests (unit + integration)
...
- **Real catalog builds** - Runs full `make build-catalogs` (2-5 min)
...
- Quick iteration during development (use `make test-scripts` instead)
...
**Slow execution**: This is expected (~5-15 min). Use fast tests for quick feedback.
